...
export const PAYMENT_DOM_WAIT_MS_MIN = Number.parseInt(
  process.env.PAYMENT_DOM_WAIT_MS_MIN ?? "3500",
  10,
);
export const PAYMENT_DOM_WAIT_MS_MAX = Number.parseInt(
  process.env.PAYMENT_DOM_WAIT_MS_MAX ?? "7000",
  10,
);
...
/** Extra pause before a retry when the failure looks like HTTP 403 (uniform random in range). */
export const CLI_RETRY_DELAY_403_MS_MIN = Number.parseInt(
  process.env.CLI_RETRY_DELAY_403_MS_MIN ?? "2000",
  10,
);
export const CLI_RETRY_DELAY_403_MS_MAX = Number.parseInt(
  process.env.CLI_RETRY_DELAY_403_MS_MAX ?? "3000",
  10,
);

/** Pick delay before next CLI attempt: longer random band after 403, else default. */
export function resolveCliRetryDelayAfterError(err) {
  const msg = err instanceof Error ? err.message : String(err);
  if (/\b403\b/.test(msg) || /HTTP\s*403/i.test(msg)) {
